...
# region Blender engine

class _BlenderCoordinate(Coordinate):
    """
    This class holds coordinates of a Blender object.
    """
    ...
    def __init__(self, blender_object: bpy.types.Object):
        self._object = blender_object

    @property
    def x(self) -> float:
        return self._object.location[0]

    @x.setter
    def x(self, x: float):
        self._object.location[0] = x

    @property
    def y(self) -> float:
        return self._object.location[1]

    @y.setter
    def y(self, y: float):
        self._object.location[1] = y

    @property
    def z(self) -> float:
        return self._object.location[2]

    @z.setter
    def z(self, z: float):
        self._object.location[2] = z

    def get_axis(self, axis: typing.ForwardRef("Axis")) -> float:
        return self._object.location[self._axis_map[axis]]

    def set_axis(self, axis: typing.ForwardRef("Axis"), value: float):
        self._object.location[self._axis_map[axis]] = value


@dataclasses.dataclass
class _BlenderMesh(LoadedMesh):
    """
    A Blender mesh is a root object and any objects in the hierarchy parented to the root object.
    """
    ...
    @name.setter
    def name(self, name: str):
        self.root_object.name = name

    def get_size_submesh_fallback(self, name: str) -> Volume:
        for object in self.objects:
            if object.name == name and object.type == "MESH":
                min_x = float("inf")
                max_x = float("-inf")
                min_y = float("inf")
                max_y = float("-inf")
    ...
    @property
    def translation(self) -> Coordinate:
        return _BlenderCoordinate(
            self.root_object
        )

    @translation.setter
    def translation(self, translation: Coordinate):
        self.root_object.location[0] = translation.x
        self.root_object.location[1] = translation.y
        self.root_object.location[2] = translation.z


class _BlenderOperatorLogger(BroomrocketLogger):
    """
    This logger logs via the Blender operator API.
    """

    def __init__(self, operator: bpy.types.Operator):
        self._operator = operator

    def debug(self, message: str):
        self._operator.report({'OPERATOR'}, message)
...
class BlenderEngine(Engine):
    """
    The Blender engine interfaces with Blender.
    """

    def _find_ancestor(self, object):
        if object.parent is None:
            return object
        return self._find_ancestor(object.parent)

    def _find_children(self, object) -> typing.List:
        return list(filter(lambda obj: self._find_ancestor(obj).name == object.name, bpy.context.scene.objects))

    def list_objects(self) -> typing.List[LoadedMesh]:
        root_objects = list(filter(lambda obj: obj.parent is None, bpy.context.scene.objects))
        return list(map(lambda obj: _BlenderMesh(obj, self._find_children(obj)), root_objects))

    def load_gltf(self, name: str, data: GLTF, logger: BroomrocketLogger) -> LoadedMesh:
        logger.debug("Loading GLTF from data stream...")
        existing_objects = []
        for o in bpy.context.scene.objects:
            existing_objects.append(o.name)
        with tempfile.TemporaryDirectory() as tempdir:
            abstempdir = os.path.abspath(tempdir)
            for file in data.files:
                if ".." in file:
                    raise BroomrocketException(False, f"Malicious GLTF file detected, path traversal in file path: {file}")
                target_path = os.path.abspath(os.path.join(abstempdir, file))
                if not target_path.startswith(abstempdir):
                    raise BroomrocketException(
                        False,
                        f"Malicious GLTF file detected, path traversal in resulting target path: {target_path}"
                    )
                logger.debug(f"Creating directory {os.path.dirname(target_path)}...")
                os.makedirs(os.path.dirname(target_path), exist_ok=True)
                logger.debug(f"Writing file to {target_path}...")
                with open(target_path, "w+b") as fh:
                    fh.write(data.files[file])
                    fh.flush()
            if ".." in data.gltf_file:
                raise BroomrocketException(
                    False,
                    "Malicious GLTF file detected, path traversal in main GLTF file: {data.gltf_file}"
                )
            gltf_path = os.path.abspath(os.path.join(abstempdir, data.gltf_file))
            if not gltf_path.startswith(abstempdir):
                raise BroomrocketException(
                    False,
                    f"Malicious GLTF file detected, path traversal in main resulting GLTF file: {gltf_path}"
                )
            logger.debug(f"Loading GLTF file from {gltf_path}...")
            bpy.ops.import_scene.gltf(filepath=gltf_path, import_pack_images=True)
        new_objects = []
        for o in bpy.context.scene.objects:
            if o.name not in existing_objects:
                new_objects.append(o)
        bpy.ops.object.empty_add(location=(0, 0, 0))
        root_empty = bpy.context.view_layer.objects.active
        root_empty.name = name
        root_empty.empty_display_type = 'PLAIN_AXES'

        for object in new_objects:
            if object.parent is None:
                object.parent = root_empty
        new_objects.append(root_empty)

        return _BlenderMesh(root_empty, new_objects)
...
dummy = DummyMeshProvider()
Sketchfab = SketchfabMeshProvider()
local = LocalMeshProvider()
broomrocket = Broomrocket(
    ...
    SpaCyNLPProvider(),
    [
        NamedReferenceFinder()
    ],
    [
        BehindPlacementStrategy(),
        FrontPlacementStrategy(),
        LeftPlacementStrategy(),
        RightPlacementStrategy(),
        AbovePlacementStrategy(),
        OnPlacementStrategy(),
        UnderPlacementStrategy(),
        NoPlacementStrategy()
    ]
)
...
class Broomrocket_UL_Sentences(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname):
        layout.prop(item, "name", text="", emboss=False, icon_value=icon)


class Broomrocket_PG_Sentences(bpy.types.PropertyGroup):
    pass


def _Sketchfab_licenses():
    licenses = list(
        map(lambda license: (license.id, license.name, license.description.rstrip(".")), Sketchfab.licenses()))
    licenses.insert(0, ('NONE', 'No filter', ''))
    return licenses


def _mesh_providers():
    providers = []
    for id, name in broomrocket.mesh_providers().items():
        providers.append((id, name, ''))
    return providers


class Broomrocket_PG_Local(bpy.types.PropertyGroup):
    root: bpy.props.StringProperty(name=f"Directory", subtype="DIR_PATH")


class Broomrocket_PG_Sketchfab(bpy.types.PropertyGroup):
    apikey: bpy.props.StringProperty(name=f"API token", subtype="PASSWORD")
    license: bpy.props.EnumProperty(
        name="License",
        default='NONE',
        items=_Sketchfab_licenses(),
    )
    savelicenses: bpy.props.BoolProperty(
        name="Save license information to text editor",
        description="Save license information about the downloaded meshes into Sketchfab-licenses.txt "
                    "in the text editor",
        default=True
    )


class Broomrocket_PG_Main(bpy.types.PropertyGroup):
    mesh_provider: bpy.props.EnumProperty(
        name="Mesh provider",
        default='local',
        items=_mesh_providers(),
    )
    sentences: bpy.props.CollectionProperty(type=Broomrocket_PG_Sentences)
    sentence_index: bpy.props.IntProperty(name="Sentence")
    prompt: bpy.props.StringProperty(name="Prompt")
    sketchfab: bpy.props.PointerProperty(type=Broomrocket_PG_Sketchfab)
    local: bpy.props.PointerProperty(type=Broomrocket_PG_Local)
    recording: bpy.props.BoolProperty()


class Broomrocket_PT_Main(bpy.types.Panel):
    bl_label = "Text to Scene (Broomrocket)"
    bl_idname = "Broomrocket_PT_Main"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_context = "objectmode"
    bl_category = "Text to Scene"

    def draw(self, context):
        layout = self.layout

        layout.prop(context.scene.broomrocket_props, 'mesh_provider', icon="MESH_DATA")

        if context.scene.broomrocket_props.mesh_provider == "Sketchfab":
            box = layout.box()
            box.prop(context.scene.broomrocket_props.sketchfab, "apikey", icon="KEYINGSET")
            box.prop(context.scene.broomrocket_props.sketchfab, "license", icon="FILTER")
            box.prop(context.scene.broomrocket_props.sketchfab, "savelicenses")
        elif context.scene.broomrocket_props.mesh_provider == "local":
            box = layout.box()
            box.prop(context.scene.broomrocket_props.local, "root", icon="FILE_FOLDER")

        row = layout.row()
        row.prop(context.scene.broomrocket_props, 'prompt')
        row.operator('broomrocket.record', icon='REC', text="")
        row.operator('broomrocket.add', icon='ADD', text="")
        row.operator('broomrocket.addrun', icon='PLAY', text="")

        row = layout.row()
        row.template_list(
            'Broomrocket_UL_Sentences', 'sentences',
            context.scene.broomrocket_props, 'sentences',
            context.scene.broomrocket_props, 'sentence_index',
            columns=1,
            sort_lock=True,
        )

        col = row.column(align=True)
        col.operator("broomrocket.remove", icon='REMOVE', text="")
        col.operator("broomrocket.removeall", icon='TRASH', text="")
        col.separator()
        col.operator("broomrocket.up", icon='TRIA_UP', text="")
        col.operator("broomrocket.down", icon='TRIA_DOWN', text="")

        row = layout.row()
        row.operator("broomrocket.save", icon="PASTEDOWN")
        row.operator("broomrocket.load", icon="COPYDOWN")

        row = layout.row()
        row.operator("broomrocket.run", icon='PLAY')
        row.operator("broomrocket.runall", icon='FF')

        layout.label(text="Copyright © Something Pink")


class Broomrocket_OP_AddAbstract(bpy.types.Operator):
    def add(self, context, data):
        sentences = re.split('([.!?])', data)
        sentence_list = []
        for sentence_text in sentences:
            sentence_text = sentence_text.strip()
            self.report({'INFO'}, sentence_text)
            if sentence_text == "." or sentence_text == "!" or sentence_text == "?":
                if len(sentence_list) > 0:
                    sentence_list[len(sentence_list) - 1] += sentence_text
            elif sentence_text != "":
                sentence_list.append(sentence_text)
        for sentence_text in sentence_list:
            sentence = context.scene.broomrocket_props.sentences.add()
            sentence.name = sentence_text
            sentence.coll_id = len(context.scene.broomrocket_props.sentences)


class Broomrocket_OP_Load(Broomrocket_OP_AddAbstract):
    bl_idname = 'broomrocket.load'
    bl_label = 'Load'
    bl_options = {'INTERNAL'}
    bl_description = 'Load sentences from file'
    ...
    def execute(self, context):
        try:
            ...
            self.report({'ERROR_INVALID_INPUT'}, f'Failed to open file: {e.__str__()}')
            return {'CANCELLED'}
        self.add(context, data)
        return {'FINISHED'}


class Broomrocket_OP_Save(bpy.types.Operator):
    bl_idname = 'broomrocket.save'
    bl_label = 'Save'
    bl_options = {'INTERNAL'}
    bl_description = 'Save sentences to file'
    ...
    def execute(self, context):
        sentences = []
        for sentence in context.scene.broomrocket_props.sentences:
...
class Broomrocket_OP_RunAbstract(bpy.types.Operator):
    def run_sentence(self, context, sentence_id: int) -> bool:
        # TODO split into invoke/execute to allow for better UI responsiveness.
        sentence = context.scene.broomrocket_props.sentences[sentence_id].name
        try:
            self.report({'INFO'}, f'Processing sentence: {sentence}')
            logger = _BlenderOperatorLogger(self)

            mesh_provider = context.scene.broomrocket_props.mesh_provider
            mesh_provider_parameters = {}
            license_file = None
            if mesh_provider == "Sketchfab":
                if context.scene.broomrocket_props.sketchfab.license is not None and \
                        context.scene.broomrocket_props.sketchfab.license != "NONE":
                    mesh_provider_parameters["license"] = context.scene.broomrocket_props.sketchfab.license
                mesh_provider_parameters["apikey"] = context.scene.broomrocket_props.sketchfab.apikey
                if context.scene.broomrocket_props.sketchfab.savelicenses:
                    license_file = "Sketchfab-licenses.txt"
            elif mesh_provider == "local":
                mesh_provider_parameters["root"] = context.scene.broomrocket_props.local.root

            license_info = broomrocket.run(
                mesh_provider,
                mesh_provider_parameters,
                sentence,
                logger
            ).strip()
            if license_info != "" and license_file is not None:
                if license_file not in bpy.data.texts:
                    bpy.data.texts.new(license_file)
                bpy.data.texts[license_file].from_string(bpy.data.texts[license_file].as_string() + "\n" + license_info)
            self.report({'INFO'}, f'Sentence processed: {sentence}')
            return True
        except BroomrocketException as e:
            self.report({'ERROR'}, f'Failed to process sentence: {sentence} ({str(e)})')
            return e.changes()


class Broomrocket_OP_Add(Broomrocket_OP_AddAbstract):
    bl_idname = 'broomrocket.add'
    bl_label = 'Add'
    bl_options = {'INTERNAL'}
    bl_description = 'Add sentence to the list'

    @classmethod
    def poll(cls, context):
        return context.scene.broomrocket_props.prompt != ""

    def execute(self, context):
        self.add(context, context.scene.broomrocket_props.prompt)
        context.scene.broomrocket_props.prompt = ""
        return {'FINISHED'}


class Broomrocket_OP_Record(Broomrocket_OP_RunAbstract, Broomrocket_OP_AddAbstract):
    bl_idname = 'broomrocket.record'
    bl_label = 'Record'
    bl_options = {'INTERNAL'}
    bl_description = 'Record audio'
    ...
    def execute(self, context):
        try:
            ...
            with mic as source:
                ...
                self.add(context, r.recognize_google(audio))
                self.run_sentence(context, len(context.scene.broomrocket_props.sentences) - 1)
            return {'FINISHED'}
        except Exception as e:
            logger = _BlenderOperatorLogger(self)
            logger.error(str(e))
...
class Broomrocket_OP_AddRun(Broomrocket_OP_RunAbstract):
    bl_idname = 'broomrocket.addrun'
    bl_label = 'Add and run'
    bl_options = {'INTERNAL'}
    bl_description = 'Add sentence to the list and run immediately'
    ...
    def execute(self, context):
        sentence = context.scene.broomrocket_props.sentences.add()
        sentence.name = context.scene.broomrocket_props.prompt
        sentence.coll_id = len(context.scene.broomrocket_props.sentences)
        context.scene.broomrocket_props.prompt = ""

        if self.run_sentence(context, len(context.scene.broomrocket_props.sentences) - 1):
            return {'FINISHED'}
        else:
            return {'CANCELLED'}


class Broomrocket_OP_Remove(bpy.types.Operator):
    bl_idname = 'broomrocket.remove'
    bl_label = 'Remove selected'
    bl_options = {'INTERNAL'}
    bl_description = 'Remove the current sentence'

    @classmethod
    def poll(cls, context):
        return -1 < context.scene.broomrocket_props.sentence_index < len(context.scene.broomrocket_props.sentences)

    def execute(self, context):
        try:
            context.scene.broomrocket_props.sentences[context.scene.broomrocket_props.sentence_index]
        except IndexError:
            return {'CANCELLED'}
        context.scene.broomrocket_props.sentences.remove(context.scene.broomrocket_props.sentence_index)
        context.scene.broomrocket_props.sentence_index = -1
        return {'FINISHED'}


class Broomrocket_OP_RemoveAll(bpy.types.Operator):
    bl_idname = 'broomrocket.removeall'
    bl_label = 'Remove all'
    bl_options = {'INTERNAL'}
    bl_description = 'Remove all sentences'

    @classmethod
    def poll(cls, context):
        return len(context.scene.broomrocket_props.sentences) > 0

    def invoke(self, context, event):
        return context.window_manager.invoke_confirm(self, event)

    def execute(self, context):
        context.scene.broomrocket_props.sentences.clear()
        context.scene.broomrocket_props.sentence_index = -1
        return {'FINISHED'}


class Broomrocket_OP_Up(bpy.types.Operator):
    bl_idname = 'broomrocket.up'
    bl_label = 'Move up'
    bl_options = {'INTERNAL'}
    bl_description = 'Move current sentence up'

    @classmethod
    def poll(cls, context):
        return 0 < context.scene.broomrocket_props.sentence_index < len(context.scene.broomrocket_props.sentences)

    def execute(self, context):
        try:
            previous_sentence = context.scene.broomrocket_props.sentences[context.scene.broomrocket_props.sentence_index - 1].name
            context.scene.broomrocket_props.sentences.move(
                context.scene.broomrocket_props.sentence_index,
                context.scene.broomrocket_props.sentence_index - 1
            )
            context.scene.broomrocket_props.sentence_index -= 1
...
class Broomrocket_OP_Down(bpy.types.Operator):
    bl_idname = 'broomrocket.down'
    bl_label = 'Move down'
    bl_options = {'INTERNAL'}
    bl_description = 'Move current sentence down'

    @classmethod
    def poll(cls, context):
        return -1 < context.scene.broomrocket_props.sentence_index < len(context.scene.broomrocket_props.sentences) - 1

    def execute(self, context):
        try:
            next_sentence = context.scene.broomrocket_props.sentences[context.scene.broomrocket_props.sentence_index + 1].name
            context.scene.broomrocket_props.sentences.move(
                context.scene.broomrocket_props.sentence_index,
                context.scene.broomrocket_props.sentence_index + 1
            )
...
class Broomrocket_OP_Run(Broomrocket_OP_RunAbstract):
    bl_idname = 'broomrocket.run'
    bl_label = 'Run'
    bl_options = {'INTERNAL'}
    bl_description = 'Run the currently selected sentence'

    @classmethod
    def poll(cls, context):
        return -1 < context.scene.broomrocket_props.sentence_index < len(
            context.scene.broomrocket_props.sentences)

    def execute(self, context):
        if self.run_sentence(context, context.scene.broomrocket_props.sentence_index):
            return {'FINISHED'}
        else:
            return {'CANCELLED'}


class Broomrocket_OP_RunAll(Broomrocket_OP_RunAbstract):
    bl_idname = 'broomrocket.runall'
    bl_label = 'Run all'
    bl_options = {'INTERNAL'}
    bl_description = 'Run all sentences'
    ...
    def execute(self, context):
        changes = False
        for i in range(0, len(context.scene.broomrocket_props.sentences)):
            if not self.run_sentence(context, i):
                return {'CANCELLED'} if not changes else {'FINISHED'}
            else:
                changes = True
        return {'FINISHED'}
...
classes = (
    Broomrocket_PG_Sentences,
    Broomrocket_PG_Sketchfab,
    Broomrocket_PG_Local,
    Broomrocket_PG_Main,
    Broomrocket_UL_Sentences,
    Broomrocket_PT_Main,
    Broomrocket_OP_Add,
    Broomrocket_OP_AddRun,
    Broomrocket_OP_Record,
    Broomrocket_OP_Remove,
    Broomrocket_OP_RemoveAll,
    Broomrocket_OP_Up,
    Broomrocket_OP_Down,
    Broomrocket_OP_Run,
    Broomrocket_OP_RunAll,
    Broomrocket_OP_Load,
    Broomrocket_OP_Save,
)
...
def register():
    ...
    for cls in classes:
        register_class(cls)
    bpy.types.Scene.broomrocket_props = bpy.props.PointerProperty(type=Broomrocket_PG_Main)


def unregister():
    del bpy.types.Scene.broomrocket_props
    del bpy.types.Scene.broomrocket
